...
import ru.utlc.exception.ServiceTypeNotFoundException;
import ru.utlc.mapper.ServiceTypeMapper;
import ru.utlc.repository.ServiceTypeRepository;
...
public class ServiceTypeService {
    ...
    @Cacheable(value = CacheNames.SERVICE_TYPES, key = "'all'")
    public Flux<ServiceTypeReadDto> findAll() {
        return serviceTypeRepository.findAll()
                .map(serviceTypeMapper::toDto)
                .doOnNext(entity -> Objects.requireNonNull(cacheManager.getCache(CacheNames.SERVICE_TYPES)).put(entity.id(), entity));
    }

    @Cacheable(value = CacheNames.SERVICE_TYPES, key = "#p0")
    ...
    @CacheEvict(value = CacheNames.SERVICE_TYPES, key = "'all'")
    @CachePut(value = CacheNames.SERVICE_TYPES, key = "#result.id")
    ...
    @CacheEvict(value = CacheNames.SERVICE_TYPES, allEntries = true)
