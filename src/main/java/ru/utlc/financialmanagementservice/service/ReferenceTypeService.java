...
import ru.utlc.financialmanagementservice.exception.ReferenceTypeNotFoundException;
import ru.utlc.financialmanagementservice.mapper.ReferenceTypeMapper;
import ru.utlc.financialmanagementservice.repository.ReferenceTypeRepository;

import java.util.Objects;

import static ru.utlc.financialmanagementservice.constants.CacheNames.REFERENCE_TYPES;
...
public class ReferenceTypeService {
    ...
    @Cacheable(value = REFERENCE_TYPES, key = "'all'")
    public Flux<ReferenceTypeReadDto> findAll() {
        return referenceTypeRepository.findAll()
                .map(referenceTypeMapper::toDto)
                .doOnNext(entity -> Objects.requireNonNull(cacheManager.getCache(REFERENCE_TYPES)).put(entity.id(), entity));
    }

    @Cacheable(value = REFERENCE_TYPES, key = "#p0")
    ...
    @CacheEvict(value = REFERENCE_TYPES, key = "'all'")
    @CachePut(value = REFERENCE_TYPES, key = "#result.id")
    ...
    @CacheEvict(value = REFERENCE_TYPES, allEntries = true)
