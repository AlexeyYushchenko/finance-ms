...
public class PaymentInvoiceService {
    ...
    public Flux<PaymentInvoiceReadDto> findAllByClientId(Integer clientId) {
        return paymentInvoiceRepository.findAllByClientId(clientId)
                .switchIfEmpty(Mono.error(new AllocationNotFoundException("error.paymentInvoice.client.notFound", clientId)))
                .map(paymentInvoiceMapper::toDto);
    }
    ...
    public Mono<PaymentInvoiceReadDto> allocatePaymentToInvoice(PaymentInvoiceCreateDto createDto) {
        // 1) Convert CreateDto -> CreateUpdateDto
        // 2) Enrich with exchangeRate from DB (fresh fetch) + do the “RUB->FOREIGN” check
        // 3) Validate + Save
        return Mono.just(paymentInvoiceMapper.toCreateUpdateDto(createDto))
                .flatMap(this::enrichDtoForCreation)                          // creation path
                .flatMap(enrichedDto -> validateAllocation(enrichedDto, null))      // no existing allocation
    ...
    public Mono<Void> deallocatePaymentFromInvoice(Long allocationId) {
        return paymentInvoiceRepository.findById(allocationId)
                ...
                .flatMap(paymentInvoiceRepository::delete)
                .as(transactionalOperator()::transactional);
    }
    ...
    private void validatePaymentAndInvoiceBelongToTheSameClient(PaymentReadDto paymentDto, InvoiceReadDto invoiceDto) {
        if (paymentDto.clientId().compareTo(invoiceDto.clientId()) != 0) {
    ...
    private void validateConvertedAmount(BigDecimal convertedAmount) {
        ...
        if (convertedAmount.compareTo(BigDecimal.valueOf(0.01)) < 0) {
            throw new ValidationException("validation.paymentInvoice.convertedAmount.min");
        }
    }
    ...
    private Mono<BigDecimal> fetchForeignToRubRate(Integer foreignCurrencyId, LocalDate paymentDate) {
        return exchangeRateService.getExchangeRate(foreignCurrencyId, RUB_CURRENCY_ID, paymentDate)
                .switchIfEmpty(
                        currencyRateService.fetchAndSaveRates(paymentDate)
                                .then(exchangeRateService.getExchangeRate(foreignCurrencyId, RUB_CURRENCY_ID, paymentDate))
                                .switchIfEmpty(Mono.error(new ExchangeRateRetrievalFailedException(
                                        "error.exchangeRate.retrievalFailed",
                                        foreignCurrencyId, RUB_CURRENCY_ID, paymentDate
                                )))
                );
    }
