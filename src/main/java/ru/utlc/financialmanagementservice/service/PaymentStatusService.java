...
import ru.utlc.financialmanagementservice.mapper.PaymentStatusMapper;
import ru.utlc.financialmanagementservice.repository.PaymentStatusRepository;

import java.util.Objects;

import static ru.utlc.financialmanagementservice.constants.CacheNames.PAYMENT_STATUSES;
...
public class PaymentStatusService {
    ...
    @Cacheable(value = PAYMENT_STATUSES, key = "'all'")
    public Flux<PaymentStatusReadDto> findAll() {
        return paymentStatusRepository.findAll()
                .map(paymentStatusMapper::toDto)
                .doOnNext(dto -> Objects.requireNonNull(cacheManager.getCache(PAYMENT_STATUSES))
                        .put(dto.id(), dto));
    }

    @Cacheable(value = PAYMENT_STATUSES, key = "#p0")
    ...
    @CacheEvict(value = PAYMENT_STATUSES, key = "'all'")
    @CachePut(value = PAYMENT_STATUSES, key = "#result.id")
    ...
    @CacheEvict(value = PAYMENT_STATUSES, allEntries = true)
